<?php
 ...
 <!-- Registered Traffic Police table data load -->
 <?php
   $user_name   = $_SESSION['user_name'];
   $db          = new Database();
   $sql         = "SELECT * FROM tb_user WHERE user_type = 'Traffic Police'";
   $read_traffic_police = $db->select($sql);
  ?>
...
<html class="fixed">
	<head>
		...
		<title>TrafficOFFENSE  - Traffic Police List</title>
					...
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Traffic Police List</span></li>
							</ol>
							...
							<div class="col-lg-12">
								<section class="panel">
									<header class="panel-heading">
										...
										<h2 class="panel-title">Traffic Police List</h2>
									</header>
									...
									<div class="panel-body">
										...
										<table class="table table-bordered table-striped mb-none" id="datatable-tabletools" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
											...
											<tbody>
												...
												<?php if($read_traffic_police){ $i = 0; ?>
                            					<?php while($result = $read_traffic_police->fetch_assoc()){ $i = $i + 1; ?>
												<tr>
													<td class="text-primary"><?php echo '[PID-'.$result['user_id'].']'; ?></td>
													<td><?php echo $result['name']; ?></td>
													<td><?php echo $result['email']; ?></td>
													<td><?php echo $result['phone']; ?></td>
													<td><?php echo 'NID-'.$result['nid']; ?></td>
													<td><?php echo $result['address']; ?></td>
													<td class="text-primary" style="font-weight: bold;"><?php echo $result['thana']; ?></td>
													<td><?php echo date("F j, Y", strtotime($result['entry_time'])) ?></td>
													<td>
				                                        <?php
				                                            $account_verify_status = $result['account_verify_status'];
													...
													<td>
														...
														<?php
				                                            ...
				                                            {
				                                              ?>
				                                              <a href="police_account_verification.php?user_id=<?php echo $result['user_id']; ?>" class="btn btn-success btn-sm" onclick="return confirm('Sure to verify account?')">Verify</a>
				                                              <?php
				                                            }
				                                          ?>
													</td>
													<td><a href="delete_data.php?police_id=<?php echo $result['user_id']; ?>" onclick="return confirm('Sure to delete?')" title="delete" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></a></td>
												</tr>
												<?php } ?>
					                            <?php }else{ ?>
